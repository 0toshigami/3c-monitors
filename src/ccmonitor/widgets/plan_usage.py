...
class PlanUsagePanel(Widget):
    ...
    def _update_limit(
        ...
        bar = self.query_one(bar_id, StyledBar)
        ...
        # Color the label with status dot based on usage
        if pct >= 80:
            prefix = "[#FF5555]\u25cf[/]"
        ...
        label.update(f"{prefix} {limit.label}: {pct:.0f}%")
